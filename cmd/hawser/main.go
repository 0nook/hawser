...
func printHelp() {
	...
	fmt.Println("    TOKEN       Authentication token (required for non-loopback binds)")
	fmt.Println("    TLS_CERT    Path to TLS certificate")
	fmt.Println("    TLS_KEY     Path to TLS private key")
	fmt.Println("    ALLOW_INSECURE_NO_AUTH  Permit a non-loopback bind with no TOKEN (insecure)")
	fmt.Println()
	fmt.Println("  Common:")
